...
with_dq as (
    select
        *,
        array_to_string(
            list_filter([
                case when created_at_ts  is null then 'unparseable_timestamp' end,
                case when premium_amount is null then 'unparseable_premium'   end,
                case when premium_amount < 0     then 'negative_premium'      end,
                case when charged_party  is null then 'missing_party'         end
            ], x -> x is not null),
            ','
        )                                   as _dq_flags,
